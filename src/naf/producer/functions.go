...
func EapEncodeAttribute(attributeType string, data string) (string, error) {
	...
	case "AT_RAND":
		...
		attrNum := fmt.Sprintf("%02x", naf_context.AT_RAND_ATTRIBUTE)
	...
	case "AT_AUTN":
		...
		attrNum := fmt.Sprintf("%02x", naf_context.AT_AUTN_ATTRIBUTE)
		attribute = attrNum + "05" + "0000" + data
	...
	case "AT_KDF":
		...
		attrNum := fmt.Sprintf("%02x", naf_context.AT_KDF_ATTRIBUTE)
		attribute = attrNum + "01" + "0001"

	case "AT_MAC":
		// Pad MAC value with 16 bytes of 0 since this is just for the calculation of MAC
		attrNum := fmt.Sprintf("%02x", naf_context.AT_MAC_ATTRIBUTE)
...
func decodeResMac(packetData []byte, wholePacket []byte, Kautn string) ([]byte, bool) {
	...
	for i := 0; i < len(dataArray); i += attributeLength {
		...
		if attributeType == naf_context.AT_RES_ATTRIBUTE {
		...
		} else if attributeType == naf_context.AT_MAC_ATTRIBUTE {
...
func ConstructFailEapAkaNotification(oldPktId uint8) string {
	...
	eapPkt.Type = naf_context.EAP_AKA_PRIME_TYPENUM
	attrNum := fmt.Sprintf("%02x", naf_context.AT_NOTIFICATION_ATTRIBUTE)
...
func getUdmUrl(nrfUri string) string {
	...
	res, err := consumer.SendSearchNFInstances(nrfUri, models.NfType_UDM, models.NfType_NAF, nfDiscoverParam)
	if err != nil {

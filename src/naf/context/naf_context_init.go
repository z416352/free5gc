...
func InitNafContext(context *NAFContext) {
	config := factory.NafConfig
	logger.InitLog.Infof("nafconfig Info: Version[%s] Description[%s]\n", config.Info.Version, config.Info.Description)
	...
	context.SBIPort = 23139                                        // default port
	if sbi != nil {
		if sbi.RegisterIPv4 != "" {
	...
	fmt.Println("naf context = ", context)
}

func AddNfServices(serviceMap *map[models.ServiceName]models.NfService, config *factory.Config, context *NAFContext) {
	...
	nfService.ServiceName = models.ServiceName_NNAF_AUTH
	...
	services[models.ServiceName_NNAF_AUTH] = nfService
}
